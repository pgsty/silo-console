...
- Recorded the maintained dependency release chain in `hack/deps-release.json` and added `hack/deps-release-check.sh`: `structural` runs in `make verifiers`; `online` (candidate gate and tag preflight) requires every tag to dereference to the recorded commit, enforces either a source-only module tag or an immutable calendar release with assets, and verifies each module through `proxy.golang.org` and `sum.golang.org` from an empty module cache against `go.sum`. The coordinated line directly requires released `silo-pkg` v3.13.0 and selects immutable `pgsty/mc` `RELEASE.2026-09-01T00-00-00Z` by its Go pseudo-version
