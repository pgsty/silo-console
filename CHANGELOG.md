...
- Regenerated `CREDITS` from the union of the Go modules linked on every release target (crediting `pgsty/mc`, `pgsty/silo-go` and `pgsty/silo-pkg` as the providers of the replaced import paths, with their own license and notice files), the production frontend dependency closure and the bundled Inter and Chakra Petch fonts; `go run ./hack/credits check` fails on a stale inventory, a module without a legal file or a stale exception. Rewrote `NOTICE` with the MinIO, Alevsk and Georg Mangold provenance chain and the PGSTY-authored modification statement
- Added `hack/verify-release-artifacts.sh` (matrix, file lists, byte-identical legal files, package license metadata, DEP-5, image labels, embedded texts) and the `release-artifacts` CI job; restructured the release workflow to build once, push the image to a verified-private staging package, verify the draft assets and the staging index, and publish from a protected environment by copying the verified index digest and un-drafting the release, with the previous `latest` digest recorded for rollback
Release and CI gates:

- Recorded the maintained dependency release chain in `hack/deps-release.json` and added `hack/deps-release-check.sh`: `structural` runs in `make verifiers` and warns while a recorded release is still pending, `online` (candidate gate and tag preflight) requires every tag to dereference to the recorded commit, enforces the release shape (a source-only module tag, or a calendar pre-release with no assets that is not latest), and verifies each module through `proxy.golang.org` and `sum.golang.org` from an empty module cache against `go.sum`. Console cannot be tagged while `silo-pkg` v3.12.3 and the `pgsty/mc` compatibility tag are pending; the compatibility strategy and the deferred own-module-path migration are documented in docs/Release.md and docs/Embedding.md
- Added the `release-candidate` workflow, dispatched from `main` for an exact commit: dependency releases, Console checks with the release asset commands, the upstream floor, the README-block embedder, and the SILO server built and tested (`go test ./cmd/`) on the coherent graph with the candidate embedded; it publishes the `release-candidate/gate` commit status and a certification record that the tag preflight verifies against the tagged SHA, run and attempt. `hack/upstream-floor.sh` and `hack/embedder-compat.sh` are shared with the pull-request matrix
- Added `e2e/version-metadata.unit.ts` (package version, generated version module and newest CHANGELOG heading must agree; final at release time) and `e2e/license-page.spec.ts` (the License page renders the embedded version); the release preflight and the candidate gate run the metadata check with `RELEASE_METADATA_MUST_BE_FINAL=1`
