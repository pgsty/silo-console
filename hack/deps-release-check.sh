#!/usr/bin/env bash
# Verify that Console's three SILO replacements point at maintained, released
# dependency versions, as recorded in hack/deps-release.json.
#
...
test "$count" -eq 3 || fail "record must describe exactly three modules, has $count"

for i in $(seq 0 $((count - 1))); do
  m() { jq -r ".modules[$i].$1 // empty" "$record"; }
  import=$(m import); replacement=$(m replacement); version=$(m version)
  ...
  case "$kind" in
    module_tag)
      ...
      ;;
    calendar_prerelease)
      is_pseudo "$version" || { echo "$import: calendar_prerelease version $version must be a pseudo-version" >&2; problems=$((problems + 1)); }
  ...
  if [ "$pending" = "true" ] && [ "$mode" = "structural" ]; then
    ...
    test "$actual" = "$replacement $current_pin" || { echo "$import: go.mod selects '$actual', record expects pending pin '$replacement $current_pin'" >&2; problems=$((problems + 1)); }
  ...
  test "$actual" = "$replacement $version" || { echo "$import: go.mod selects '$actual', record expects '$replacement $version'" >&2; problems=$((problems + 1)); continue; }
  ...
  case "$kind" in
    module_tag)
      ...
      ;;
    calendar_prerelease)
      [ -n "$release" ] || { echo "$import: calendar_prerelease $tag must be a published GitHub release" >&2; problems=$((problems + 1)); continue; }
      jq -e '.draft == false and .prerelease == true and (.assets | length) == 0' <<<"$release" >/dev/null \
        || { echo "$import: $tag must be published, marked pre-release and carry no assets" >&2; problems=$((problems + 1)); }
      latest=$(gh api "repos/$repository/releases/latest" --jq '.tag_name' 2>/dev/null || true)
      test "$latest" != "$tag" || { echo "$import: $tag must not be the latest release of $repository" >&2; problems=$((problems + 1)); }
      ;;
  esac
  ...
  info=$(cd "$work" && env $(isolated_env) go mod download -json "$replacement@$version") \
    || { echo "$import: proxy.golang.org cannot serve $replacement@$version" >&2; problems=$((problems + 1)); continue; }
  ...
  escaped=$(printf '%s' "$replacement" | sed 's/[A-Z]/!\L&/g')
  ...
  lookup=$(curl -fsS "https://sum.golang.org/lookup/$escaped@$version") \
    || { echo "$import: sum.golang.org has no record for $replacement@$version" >&2; problems=$((problems + 1)); continue; }
  grep -qF "$replacement $version $sum" <<<"$lookup" || { echo "$import: checksum database module hash differs from the downloaded module" >&2; problems=$((problems + 1)); }
  grep -qF "$replacement $version/go.mod $gomodsum" <<<"$lookup" || { echo "$import: checksum database go.mod hash differs from the downloaded module" >&2; problems=$((problems + 1)); }
  grep -qF "$replacement $version $sum" go.sum || { echo "$import: go.sum lacks '$replacement $version $sum'" >&2; problems=$((problems + 1)); }
  grep -qF "$replacement $version/go.mod $gomodsum" go.sum || { echo "$import: go.sum lacks the go.mod hash for $replacement $version" >&2; problems=$((problems + 1)); }
  echo "$import: $replacement@$version verified ($kind, $repository@$tag = $commit, proxy + sum.golang.org + go.sum agree)"
done
