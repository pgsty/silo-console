...
type MinioAdmin interface {
	...
	delConfigKV(ctx context.Context, kv string) (restart bool, err error)
...
func (ac AdminClient) delConfigKV(ctx context.Context, kv string) (restart bool, err error) {
...
func getClientIP(r *http.Request) string {
	return clientIPFromSource(r, getSourceIPFromHeaders(r))
}

func clientIPWithPolicy(r *http.Request, policy sourceIPTrustPolicy) string {
	return clientIPFromSource(r, sourceIPFromHeaders(r, policy))
}

func clientIPFromSource(r *http.Request, addr string) string {
	if addr == "" {
		addr = r.RemoteAddr
	}
